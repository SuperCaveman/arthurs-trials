...
const apiLogPath = join(root, 'session-api.log');
...
await writeFile(serverLogPath, [
  ...
  '[2026.08.13-03.06.10:000][872] LogArthursTrialsGameServer: GameLift accepted a player session for 127.0.0.1.',
  '[2026.08.13-03.06.10:100][872] LogArthursTrialsGameServer: Bound validated GameLift player session to 127.0.0.1.',
...
await writeFile(apiLogPath, '{"event":"match_ready","matchRequestId":"mrq_redacted"}\n');
...
const evidence = await collectOperationsEvidence({ serverLogPath, outboxDirectory, apiLogPath });
...
assert.equal(evidence.sessionApi.placementRequested, true);
...
assert.match(html, /API placement issued/);
assert.match(html, /Player session admitted/);
