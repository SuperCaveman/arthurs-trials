...
| GameLift health fault injection | **Implemented; runtime proof pending** | The Anywhere launcher can opt in to a bounded number of failed health checks with `-FailHealthChecks`; the server then resumes normal health callbacks. It is disabled by default and guarded by a static CI contract test. | Rebuild the Windows dedicated-server package, run one controlled Anywhere fault test, and archive its redacted lifecycle excerpt. |
