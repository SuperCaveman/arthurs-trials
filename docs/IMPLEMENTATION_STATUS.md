...
| Results-worker container | **Verified locally** | The idempotent local match-results worker has a non-root Node container artifact. It defaults to in-memory input/state, so its local build and CI image build have no AWS dependency. | Connect the artifact to the opt-in SQS/DLQ and private database foundations during an approved managed demo. |
