...
| Player authentication | **Planned** | Cognito is part of the target architecture only. | Implement an authenticated API request and verify a client cannot request another player's session. |
...
| Linux dedicated-server artifact | **Verified locally; not deployed** | UE 5.8's v26 Linux cross-toolchain produced a 64-bit x86_64 ELF server. The server was cooked, staged, packed with IoStore, archived, built into a 1.34 GB local Amazon Linux 2023 image, and smoke-tested as a non-root process bound to UDP `7777`. | Tag the tested image with a Git revision and scan it before any explicitly approved ECR push. See the [build plan](LINUX_SERVER_BUILD_PLAN.md). |
...
| Match results workflow | **Local contract verified; not deployed** | The local worker validates `match.completed` events, grants XP once per event ID, and safely reports a replay as a duplicate. Its store is intentionally in-memory; no SQS, DLQ, RDS, or worker service exists. | Connect an authoritative server event and later replace local input/store with SQS and transactional persistence. |
