...
| Match results workflow | **Verified locally; not deployed** | A real Anywhere game session carried server-only match metadata. The dedicated server published `match.completed` to a local outbox; the worker processed XP exactly once and archived the event. An opt-in local file store now proves the same event is rejected after a worker restart while preserving the original rewards. The Session API is contract-tested to pass the same metadata. No SQS, DLQ, RDS, or worker service exists. See [redacted proof](evidence/LOCAL_MATCH_RESULTS_PROOF.md). | Replace local input/store with SQS and transactional persistence. |
